...
export default function Calculator6() {
    ...
    // 孖宝缆法专用状态
    const [isOnDouble, setIsOnDouble] = useState(false); // 是否在下双倍

    // 孖宝缆法注码序列 - 18个位置，前12个是实际注码
    const mataoLevels = [20, 20, 20, 20, 30, 40, 50, 70, 90, 120, 160, 220, 0, 0, 0, 0, 0, 0];
    const [betLevels, setBetLevels] = useState([...mataoLevels]);
    const [baseLevels, setBaseLevels] = useState([...mataoLevels]);
    ...
    const addResult = (result: Result) => {
        ...
        if (lastRecommendation) {
            const currentStake = isOnDouble ? betLevels[currentBetLevel] * 2 : betLevels[currentBetLevel];
    ...
    const resetBetLevels = () => {
        setBetLevels([...mataoLevels]);
        setBaseLevels([...mataoLevels]);
    };
                            ...
                            {betLevels.map((level, index) => (
                                ...
                                >
                                    {editingLevel === index ? (
                                        ...
                                        <span>
                                            {level === 0 ? '\u00A0' : (isOnDouble && index === currentBetLevel ? level * 2 : level)}
                                            {index === currentBetLevel && isOnDouble && (
                            ...
                            <div className="space-y-3">
                                <div className="p-4 border border-blue-500 bg-blue-50 rounded-lg">
                                    <div className="flex items-center justify-between mb-2">
                                        <h4 className="font-semibold text-gray-800">孖宝缆法</h4>
                                        <div className="w-4 h-4 bg-blue-500 rounded-full flex items-center justify-center">
                                            <div className="w-2 h-2 bg-white rounded-full"></div>
                                        </div>
                                    </div>
                                    <p className="text-sm text-gray-600 mb-2">每一个数字以及这个数字的双倍都赢了就返回第一个数字</p>
                                    <div className="text-xs text-gray-500 font-mono">
                                        [20, 20, 20, 20, 30, 40, 50, 70, 90, 120, 160, 220]
                                    </div>
                                    <div className="text-xs text-blue-600 mt-2">
                                        基础→孖宝→赢了回第一个数字；任何一次输了进入下个数字
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-lg p-6 max-w-lg mx-4 max-h-[80vh] overflow-y-auto">
                            ...
                            <div className="space-y-4 text-sm text-gray-700">
                                ...
                                <section>
                                    <h4 className="font-semibold text-gray-800 mb-2">💰 孖宝缆法系统</h4>
                                    <ul className="list-disc list-inside space-y-1">
                                        <li>默认注码：20, 20, 20, 20, 30, 40, 50, 70, 90, 120, 160, 220，可以自行修改</li>
                                        <li>点击数字可编辑投注金额</li>
                                        <li>基础模式赢了→进入孖宝模式；孖宝模式赢了→回到第一级</li>
                                        <li>任何一次输了→进入下个数字的基础模式</li>
                                        <li>三行六列共18个位置，前12个为实际注码</li>
                                    </ul>
                                </section>
